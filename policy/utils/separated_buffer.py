...
from policy.utils.util import check, get_shape_from_obs_space, get_shape_from_act_space
...
class SeparatedReplayBuffer(object):
    def __init__(self, args, robot_obs_space, human_obs_space, share_obs_space, act_space):
        ...
        # robot_obs_shape = max(args.robot_obs_dim+2, args.human_obs_dim)
        # human_obs_shape = max(args.robot_obs_dim+2, args.human_obs_dim)
        robot_obs_shape = args.robot_obs_dim+2
        human_obs_shape = args.human_obs_dim
        # share_obs_shape = robot_obs_shape * (self.human_num+self.robot_num)

        # if type(robot_obs_shape[-1]) == list:
        #     robot_obs_shape = robot_obs_shape[:1]   #取原本obs_shape的第一个元素

        # if type(human_obs_shape[-1]) == list:
        #     human_obs_shape = human_obs_shape[:1]

        # if type(share_obs_shape[-1]) == list:
        #     share_obs_shape = share_obs_shape[:1]

        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, (max(self.human_num,self.att_agents)+self.robot_num), robot_obs_shape), dtype=np.float32)
        self.robot_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, robot_obs_shape), dtype=np.float32)
        self.human_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, max(self.human_num, self.att_agents), human_obs_shape), dtype=np.float32)
    ...
    def insert(self, share_obs, robot_obs, human_obs, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step + 1] = share_obs.copy()
        self.robot_obs[self.step + 1] = robot_obs.copy()
        self.human_obs[self.step + 1] = human_obs.copy()
    ...
    def chooseinsert(self, share_obs, robot_obs, human_obs, rnn_states, rnn_states_critic, actions, action_log_probs,
                     value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
    ...
    def after_update(self): #used
        self.share_obs[0] = self.share_obs[-1].copy()
        self.robot_obs[0] = self.robot_obs[-1].copy()
        self.human_obs[0] = self.human_obs[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        episode_length, n_rollout_threads = self.rewards.shape[0:2]  #通过reward的维度大小获取episode_length, n_rollout_threads大小
        ...
        robot_obs = self.robot_obs[:-1].reshape(-1, *self.robot_obs.shape[2:])
        human_obs = self.human_obs[:-1].reshape(-1, *self.human_obs.shape[2:])
        ...
        for indices in sampler:
            ...
            robot_obs_batch = robot_obs[indices]
            human_obs_batch = human_obs[indices]
            ...
            yield share_obs_batch, robot_obs_batch, human_obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
