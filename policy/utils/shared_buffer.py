import torch
import numpy as np
from policy.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        # store useful dimensions from args
        self.human_num = args.num_humans
        self.human_obs_dim = args.human_obs_dim
        self.robot_obs_dim = args.robot_obs_dim + 2
        ...
        # derive observation layout from env args instead of hard-coding
        # per-agent observation tensor shape in env_core:
        #   (1 + max(num_humans, num_attention_agents), obs_dim + 2)
        # where obs_dim = max(robot_obs_dim, human_obs_dim)
        obs_row_dim = max(args.robot_obs_dim, args.human_obs_dim) + 2
        num_rows = 1 + max(self.human_num, self.att_agents)

        # centralized (shared) observation for each agent is the flattened joint obs
        # across all agents and rows
        share_obs_dim = num_agents * num_rows * obs_row_dim

        self.share_obs = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, share_obs_dim),
            dtype=np.float32,
        )
        self.obs = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, num_rows, obs_row_dim),
            dtype=np.float32,
        )
        ...
        self._use_attn_comm = bool(getattr(args, "use_attn_comm_actor", False)) and (
            not bool(getattr(args, "enable_dynamic_goal_assignment", False))
        )
        self.comm_state_dim = int(getattr(args, "attn_comm_state_dim", None) or self.hidden_size)
        if self._use_attn_comm:
            self.comm_rnn_states = np.zeros(
                (self.episode_length + 1, self.n_rollout_threads, num_agents, self.comm_state_dim),
                dtype=np.float32,
            )
        else:
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.comm_rnn_states is not None:
            comm_rnn = self.comm_rnn_states[:-1].reshape(-1, *self.comm_rnn_states.shape[3:])
        else:
            comm_rnn = None
        ...
        for indices in sampler:
            ...
            # obs_batch shape: (batch, R, C) where R=1+max(human_num,att_agents), C=obs_dim+2
            R = obs_batch.shape[1]
            C = obs_batch.shape[2]
            # infer dimensions: robot_obs uses first robot_obs_dim columns, human_obs uses first human_obs_dim cols
            # prefer to use attributes if present
            robot_obs_dim = getattr(self, 'robot_obs_dim', C)
            human_obs_dim = getattr(self, 'human_obs_dim', min(5, C))
            human_num = getattr(self, 'human_num', max(0, R - 1))

            robot_obs_batch = obs_batch[:, 0, :robot_obs_dim]  # (batch, robot_obs_dim)
            human_obs_batch = obs_batch[:, 1:1 + human_num, :human_obs_dim]  # (batch, human_num, human_obs_dim)
            rnn_states_batch = rnn_states[indices]
            rnn_states_critic_batch = rnn_states_critic[indices]
            if comm_rnn is not None:
                comm_rnn_batch = comm_rnn[indices]
            else:
                comm_rnn_batch = None
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        if self.comm_rnn_states is not None:
            comm_rnn = self.comm_rnn_states.reshape(-1, batch_size, self.comm_state_dim)
        else:
            comm_rnn = None
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                if comm_rnn is not None:
                    comm_rnn_batch_list.append(comm_rnn[:-1, ind])
            ...
            if comm_rnn is not None:
                comm_rnn_stack = np.stack(comm_rnn_batch_list, 1)
            else:
                comm_rnn_stack = None
            ...
            # obs_batch now shape (T*N, R, C) -> split into robot/human
            R = obs_batch.shape[1]
            C = obs_batch.shape[2]
            robot_obs_dim = getattr(self, 'robot_obs_dim', C)
            human_obs_dim = getattr(self, 'human_obs_dim', min(5, C))
            ...
            robot_obs_batch = obs_batch[:, 0, :robot_obs_dim]
            human_obs_batch = obs_batch[:, 1:1 + human_num, :human_obs_dim]
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N, *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(N, *self.rnn_states_critic.shape[3:])
            if comm_rnn_chunks is not None:
                comm_rnn_stack = np.stack(comm_rnn_chunks, axis=1)
            else:
                comm_rnn_stack = None
            ...
            # split obs_batch into robot and human parts
            R = obs_batch.shape[1]
            C = obs_batch.shape[2]
            robot_obs_dim = getattr(self, 'robot_obs_dim', C)
            human_obs_dim = getattr(self, 'human_obs_dim', min(5, C))
