import logging
from policy.policy_human import Policy
import numpy as np
from envs.utils.utils import *
import torch
...
class Net(nn.Module):
    def __init__(self, input_dim, self_state_dim, lstm_hidden_dim):
        ...
        # action network
        self.act_fc1 = nn.Linear(self_state_dim + lstm_hidden_dim, 128)  # input = self_state_dim + lstm_hidden_dim, output = 128
        # self.act_fc2 = nn.Linear(128, 128)
        # self.act_fc3 = nn.Linear(256, 256)
        self.act_fc4 = nn.Linear(128, 81)
        self.act_fc4.weight.data.mul_(0.1) # ???

        # value network
        self.value_fc1 = nn.Linear(self_state_dim + lstm_hidden_dim, 128)
    ...
    def forward(self, state):
        ...
        dim = state.dim()
        if dim == 2:
            state = state.unsqueeze(0) # add 1 dim 1*10*13
            size = state.shape
...
class LstmRL(Policy):
    ...
    def predict(self, state):
        ...
        state.human_states = sorted(state.human_states, key=dist, reverse=True)  # reverse=True ,descending order
        ...
        _, pi = self.model(states_tensor)#.data.item()  output = pi , _ = vaule
