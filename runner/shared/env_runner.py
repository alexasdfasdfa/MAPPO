...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs = self._maybe_resolve_undetermined_targets(obs)
    ...
    def warmup(self):
        ...
        obs = self._maybe_resolve_undetermined_targets(obs, force=True)
    ...
    def _maybe_resolve_undetermined_targets(self, obs, force=False):
        if not bool(getattr(self.all_args, "enable_undetermined_goal", False)):
            return obs
        if (not force) and (not self._has_pending_undetermined_in_obs(obs)):
            return obs
        n_envs = self.n_rollout_threads
        n_agents = self.num_agents
        robot_obs_dim = self.all_args.robot_obs_dim + 2
        robot_rows = obs[:, :, 0, :robot_obs_dim]
        flat_robot_obs = robot_rows.reshape(n_envs * n_agents, robot_obs_dim)
        self.trainer.prep_rollout()
        tids = self.trainer.policy.get_undetermined_targets(flat_robot_obs, deterministic=False)
        tids_np = _t2n(tids).reshape(n_envs, n_agents).astype(np.int64)
        return self.envs.apply_undetermined_targets(tids_np)

    def _has_pending_undetermined_in_obs(self, obs):
        if obs is None:
            return False
        # pending flag is the scalar right after v2 slot block: idx = 7 + 5*M.
        m = int(getattr(self.all_args, "undetermined_v2_goal_slots", 10))
        pending_idx = 7 + 5 * m
        if obs.shape[-1] <= pending_idx:
            return False
        p = obs[:, :, 0, pending_idx]
        if np.any(p > 0.5):
            return True
    ...
    def collect(self, step):
        ...
        self.trainer.prep_rollout()
        _out = self.trainer.policy.get_actions(
        ...
        if len(_out) < 5:
            raise RuntimeError(f"policy.get_actions returned unexpected tuple length: {len(_out)}")
        value, action, action_log_prob, rnn_states, rnn_states_critic = _out[:5]
        comm_rnn_out = _out[5] if len(_out) > 5 else None
        broadcast_msg = _out[6] if len(_out) > 6 else None
        ...
        comm_rnn_np = None
        if comm_rnn_out is not None:
            comm_rnn_np = np.array(np.split(_t2n(comm_rnn_out), self.n_rollout_threads))
        bmsg_np = None
        if broadcast_msg is not None:
            bmsg_np = np.array(np.split(_t2n(broadcast_msg), self.n_rollout_threads))
            self.envs.set_comm_broadcasts(bmsg_np)
        if bool(getattr(self.all_args, "enable_undetermined_v3_exchange", False)):
            try:
                ch = self.trainer.policy.get_v3_exchange_choices(robot_obs, deterministic=False)
                ch_np = _t2n(ch).reshape(self.n_rollout_threads, self.num_agents).astype(np.int64)
                self.envs.set_v3_exchange_choices(ch_np)
            except Exception:
                pass
        # rearrange action
