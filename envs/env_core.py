...
[[1,26], [5,26],[9,26],[13,26],[9,19],[5,11],[1,3],[5,3],[9,3],[13,3]]                                             ,      #Z           
            ...
            s_shape_targets = candidate_targets[4]
    ...
    def step(self):
        ...
        for i, robot in enumerate(self.robots):
            ...
            sub_agent_reward.append(self.get_reward_lgpf(robot,for_feature))
    ...
    def get_reward(self,robot,for_feature):
        ...
        #navigation
        # if robot.radius < cal_distance(robot.px,robot.py,robot.gx,robot.gy) <= robot.radius * 3:
        #     if (robot.theta - 0.35) <= np.arctan2(robot.gy - robot.py, robot.gx - robot.px) <= ((robot.theta + 0.35)):
        #         r_nav += (robot.radius * 3 - cal_distance(robot.px,robot.py,robot.gx,robot.gy)) *0.1 + robot.v
        #     if robot.pre_dist2goal - robot.dist2goal < 0.1:
        #         r_nav += (robot.pre_dist2goal - robot.dist2goal - 0.1) * 20
        ...
        discount_formation = 30
        discount_avoid = 50
        discount_nav = 20
        discount_goal = 200
        discount_bonus = 2

        reward =  discount_formation * r_formation + discount_avoid * r_avoid + discount_nav * r_nav \
    ...
    def get_reward_new(self, robot, for_feature, config=None):
        ...
        nav_scale = config.get('nav_scale', 2.0)
        r_nav = np.tanh(delta_dist / nav_scale)
        ...
        if robot.collision:
            ...
            discomfort_dist = config.get('discomfort_dist_scale', 1.0)
            r_avoid = -1.0 / (1.0 + np.exp(robot.dmin / discomfort_dist - 2.0))
        
        # ========================
        # 3. 编队奖励
        # ========================
        robot_num = len(self.robots) if self.robots else 1
        formation_scale = config.get('formation_scale', robot_num)
        if formation_scale == 0:
            formation_scale = robot_num
        r_formation = -np.tanh(for_feature / formation_scale)
        ...
        # 5. 时间惩罚
        # ========================
        r_time = config.get('time_penalty', -0.01)
        
        # ========================
        # 6. 权重
        # ========================
        w_nav = config.get('w_nav', 1.0)
        w_avoid = config.get('w_avoid', 2.0)
        w_formation = config.get('w_formation', 0.5)
